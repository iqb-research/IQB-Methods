...
1. Gleiches gilt für eine Funktion: sie sollte maximal 100 Zeilen lang sein, und eher in Sub-Funktionen aufgeteilt werden.
1. In R-Paketen sollte es pro exportierter Funktion ein eigenes Skript geben. Zu Hilsfunktionen haben wir noch nichts beschlossen.
...
1. Es sollten eher sprechende Namen als möglichst kurse Namen verwendet werden. 
1. Funktionsnamen sollten aus Verben bestehen. 
1. Objektnamen (die keine Funktionen sind) sollten Substantive sein. 
1. Dopplungen mit existierenden R-Objekten sollten vermieden werdeen. 
1. Bei der Objekt-Modifikation sollten Objekte nicht überschreiben werden, wenn sie sich substanziell ändern. 
1. Einige häufige Argumentnamen: `dat`, `path/file/filePath/filename`, `dir/folder`, ...
1. Bei der Argumentreihenfolge sollte die Pipebarkeit und Output-Steuerung beachtet werden. 
...
Eine übersicht zum GitHub-Workflow findet sich [hier](https://iqb-research.github.io/IQB-Methods/docs/git/). Eine etwas ausführlicheres Tutorial zur Einführung in GitHub, aber auch zu Pull-Requests und Code-Reviews findet sich [hier](https://iqb-research.github.io/IQB-Methods/docs/git/github_workshop/github_intro_slides.html#/title-slide). 

### Vorgehen

1. Person, die das Review anfordert, schlägt Personen für das Review for, in dem diese zur Pull-Request assigned werden.
1. Sind mehrere Reviewer zum **Review** assigned, reicht es i.d.R. wenn eine:r das Review übernimmt (es sei denn, es wird explizit angefordert, dass alle das Review übernehmen)
1. Übernimmt man ein Review, assigned man sich selbst den **Pull-Request**, dann wissen alle potenziell angesprochenen Reviewer gleich Bescheid, dass man übernimmt, auch wenn man noch nicht angefangen hat (bzw. noch keine Kommentare in den Code gesetzt hat)
...
1. Achten ob Benennungskonventionen und Formatkonventionen eingehalten wurden. 
1. Prüfen, ob der code verständlich geschrieben ist und möglichst keine erklärenden Kommentare benötigt.
1. Tests auf Vollständigkeit überprüfen (sind mögliche Edge Cases (NA, negative Werte, hohe/niedrige Werte, anderer data type ...) abgedeckt?). 
...
1. Label sollen einheitlich über die Repositories erstell und genutzt werden (Bug/Enhancement ..., Prio-Label)
