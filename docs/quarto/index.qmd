...
Basically you just need to know where to put your files. Everything else will be taken care of by pushing to GitHub. Files can go into one of two folders: `docs` or `posts`. `docs` contains most of the tutorial files: They are structured into sub folders, like `eatPackages` or `R`. Here the quarto-files can be found that contain the actual website content. Edit them or add new ones. Just make sure they are quarto-files with the `.qmd` ending.
...
To get a preview of your website, just click on the `Render` button in R Studio. This is not necessary however, because the website will only be built online when you push to github. It will take a while (up to 20 min or more, depending on the size of the website) until the website is updated, as some checks are run first. Opening a pull request is enough for the changes to be represented on the website. 
