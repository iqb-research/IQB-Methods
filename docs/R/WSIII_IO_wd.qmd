---
title: "Workshop 3: Pakete, Working Directory, Input und Output"
...
`save()` dient zur Speicherung mehrerer Objekte auf einmal, entsprechende Dateien sollten die Endung `RData` erhalten. Beim Laden dieser Objekte behalten diese ihren ursprünglichen Namen. Dies kann nervig sein, da mögliche andere Personen nicht wissen, welche Namen die gespeicherten Objekte hatten.
...
Wenn man überprüfen möchte, wie die geladenen Objekte heißen, kann man mithilfe des Befehls `ls()` die aktuell vorhanden Objekte in der globalen Umgebung überprüfen oder diese in RStudio unter `Environment` betrachten.
...
```{r import_spss, message=FALSE}
# sav Datei einlesen
spss <- eatGADS::import_spss("Material_WSIII/example.sav")
```

Diese Datei enthält sämtliche Metadaten, die auch die originale spss-Datei beinhaltet (Variablen- und Wertelabel etc.). Diese können mithilfe von `extractMeta` abgefragt werden.
