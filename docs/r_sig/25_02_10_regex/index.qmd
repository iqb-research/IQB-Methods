...
Regular expressions is a powerful language for describing patterns within strings. Mostly we need that for extracting certain strings for filtering or manipulation. See [R for Data Science](https://r4ds.hadley.nz/regexps) for a whole chapter on this. 
Also, speaking from experience, ChatGPT is a great resource on this, still it might be helpful to understand the underlying syntax a bit better. 
...
7. Read in the text file [`faust.txt`](). 
Plot the 50 most common words in Goethes Faust. 

:::{.callout-tip collapse="true"}
## Hint
The package `readtext` might help you with reading in `.txt` files.
You might want to look in the examples of the `str_extract_all()` documentation. 
It might help with extracting words!
:::
