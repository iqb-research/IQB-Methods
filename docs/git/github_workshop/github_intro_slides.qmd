...
## Merge Conflicts
In erstaunlich vielen Fällen funktioniert die "Git-Magie" von alleine, und es gibt keine Probleme beim mergen. Manchmal kommt es aber vor, dass Git nicht weiß, wie es zwei Änderungen in einer Datei zusammenführen soll, z.B. wenn vom gleichen Branch zwei verschiedene Branches abgezweigt und dann die gleiche Datei bearbeitet wurde. Dann kann es zu [Merge-Konflikten]{.highlight} kommen, die wir manuell lösen müssen.
...
:::{.callout-caution}
Pullt die Änderungen von `main`, damit ihr beide auf dem aktuellen Stand seid. Erstellt dann jeweils einen neuen Branch, auf dem ihr jetzt [in der gleichen Datei]{.highlight} in der gleichen Zeile Änderungen vornehmt. Das wird hoffentlich einen merge conflict erzeugen. 
:::

:::{.callout-caution}
## Übung
Pusht eure Änderungen, und einigt euch, wer zuerst mergt. Diese Person bekommt noch keinen Mergekonflikt. Sobald gemergt wurde, kann die andere Person mergen. Das wird einen Mergekonflikt erzeugen. Löse ihn!
:::

Wenn ihr wollt, könnt ihr danach nochmal tauschen, sodass die andere Person den Mergekonflikt lösen muss.
