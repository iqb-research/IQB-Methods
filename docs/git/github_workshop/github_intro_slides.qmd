...
## Zu diesen Folien
HTML-Folien können durch `e` in PDF umgewandelt werden, und dann mit Drucken aus dem Browser abgespeichert werden, falls ihr darin kommentieren wollt. 
...
:::{.callout-warning}
## Übung
[Clone](https://iqb-research.github.io/IQB-Methods/docs/git/github_workshop/github_intro_slides.html#/clone) das gerade erstellte Repository. Wenn du nicht RStudio dafür nutzt, erstelle außerdem ein RStudio-Projekt in dem geklonten Ordner (`File - New Project - Existing Directory`), sowie eine `.gitignore` Datei mit folgendem Inhalt: 
...
:::{.callout-warning}
## Übung
[Stage](https://iqb-research.github.io/IQB-Methods/docs/git/github_workshop/github_intro_slides.html#/stage) deine gerade bearbeitete Datei, sowie das erstellte RStudio-Projekt und die `.gitignore` Datei. Ganz sauber wäre es, erst einmal nur die `.txt` Datei zu stagen, und dann die anderen Datein, um daraus insgesamt zwei Commits zu machen.
:::
...
:::{.callout-note}
Vergiss nicht, vor der Erstellung zu pullen, damit die neuste Projekt-Version für den neuen branch genutzt wird. 
:::
...
:::{.callout-caution}
## Übung 
[Clone](https://iqb-research.github.io/IQB-Methods/docs/git/github_workshop/github_intro_slides.html#/clone) das Repository (wenn noch nicht geschehen). 
:::
...
1. Das öffnet, unabhängig vom Editor (z.B. GitHub online, VS Code), einen Texteditor, indem die Datei so bearbeiten werden kann, wie sie am Ende aussehen soll. 
1. Konflikte sind mit `<<<<<<<`, `=======` und `>>>>>>>` markiert. Wir können uns entscheiden, welche Änderungen wir behalten wollen. 
1. Die Konflikt-Marker müssen ebenfalls gelöscht werden, sodass die Datei am Ende genauso aussieht, wie sie auch commitet werden soll. 
1. Wenn fertig, klick auf `Mark as resolved`. 
1. Committen & Mergen. 
:::
