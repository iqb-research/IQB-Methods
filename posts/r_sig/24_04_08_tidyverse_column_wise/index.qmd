...
- If we just want to extract data, we can use the `data` pronoun (or the `.` as short version) with `[[` ([see](https://dplyr.tidyverse.org/articles/programming.html#indirection)).
- We can also simply write `"{var}_..."` to paste together a new column name. 
...
2. Apply this function within a for-loop on 10 columns of your choice.
...
This is a pretty hard exercise, so use google, StackOverflow and the help function `?` within R. 
